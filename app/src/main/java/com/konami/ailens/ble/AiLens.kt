...
class AiLens(private val context: Context, override var device: BluetoothDevice, private val retrieveToken: ByteArray? = null): Glasses {
    ...
    private val enterInterpretationCheck = "454dC7000100020001"
    private val leaveInterpretationCheck = "454dC7000100020000"
    private val enterDialogTranslationCheck = "454d3000"
    private val leaveDialogTranslationCheck = "454d3100"
    ...
    private val btConnectionCheck = "4f423A0003000600"

    private val micOpenedCheck = "454d2f000100020001"
        ...
        when { // Device asks to show long-press-to-pair
            ...
            value.size == 9 && value.startsWith(userPressPairingCheck) -> {
                ...
                if (ok && deviceToken != null) {
                    SharedPrefs.saveDeviceInfo(device.address, userId, deviceToken!!)
                }
...
//                    _state.value = State.CONNECTED
            ...
            value.startsWith(enterInterpretationCheck) -> {
                deviceEventHandler?.handleDeviceEvent(DeviceEventCapability.DeviceEvent.EnterSimultaneousTranslation)
            }

            value.startsWith(leaveInterpretationCheck) -> {
                deviceEventHandler?.handleDeviceEvent(DeviceEventCapability.DeviceEvent.LeaveSimultaneousTranslation)
            }
            ...
            value.startsWith(enterDialogTranslationCheck) -> {
                deviceEventHandler?.handleDeviceEvent(DeviceEventCapability.DeviceEvent.EnterDialogueTranslation)
            }

            value.startsWith(leaveDialogTranslationCheck) -> {
                deviceEventHandler?.handleDeviceEvent(DeviceEventCapability.DeviceEvent.LeaveDialogueTranslation)
            }
            ...
            value.startsWith(micOpenedCheck) -> {
                deviceEventHandler?.handleDeviceEvent(DeviceEventCapability.DeviceEvent.OpenMic)
            }
