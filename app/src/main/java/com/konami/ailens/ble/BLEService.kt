...
class BLEService private constructor(private val context: Context) {
    ...
    fun retrieve() {
        ...
        val currentSession = _connectedSession.value
        if (currentSession != null && currentSession.device.address == address) {
            val currentState = currentSession.state.value
            if (currentState == DeviceSession.State.CONNECTING ||
                currentState == DeviceSession.State.CONNECTED) {
                Log.d(TAG, "Already connecting/connected to $address, skipping retrieve()")
                return
            }
        }

        Log.d(TAG, "Creating new session for $address")
